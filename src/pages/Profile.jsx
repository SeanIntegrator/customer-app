      ...
      <AnimatePresence mode="wait">
        {display > 0 && (
          <motion.span
            ...
            style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: 11, fontWeight: 600, color: value > 0 ? '#c8902a' : 'rgba(26,46,26,0.4)' }}
          >
            {STAR_LABELS[display]}
          </motion.span>
        )}
...
function SectionHead({ label, title, action }) {
  return (
    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 14 }}>
      <div>
        <p style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: 9, fontWeight: 700, letterSpacing: '0.22em', textTransform: 'uppercase', color: '#c8902a', marginBottom: 3 }}>
          ✦ {label}
        </p>
        <h2 style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: 22, fontWeight: 800, color: '#1a2e1a', letterSpacing: '-0.02em', lineHeight: 1 }}>
          {title}
        </h2>
      </div>
      {action}
