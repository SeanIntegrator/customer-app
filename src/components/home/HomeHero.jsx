...
export default function HomeHero({
  ...
  return (
    ...
    >
      <div style={{ position: 'absolute', inset: 0, backgroundImage: PAPER_GRAIN_BACKGROUND, pointerEvents: 'none' }} />
      ...
      <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
        ...
        <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', opacity: 0.055 }} preserveAspectRatio="none">
          {[20, 40, 60, 80].map((p) => (
            <line key={`v${p}`} x1={`${p}%`} y1="0" x2={`${p}%`} y2="100%" stroke="white" strokeWidth="1" />
          ))}
          {[33, 66].map((p) => (
            <line key={`h${p}`} x1="0" y1={`${p}%`} x2="100%" y2={`${p}%`} stroke="white" strokeWidth="1" />
          ))}
          {[20, 40, 60, 80].map((x) =>
            [33, 66].map((y) => (
              <g key={`x${x}${y}`}>
      ...
      <div style={{ position: 'relative', padding: '0 24px', zIndex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.08, type: 'spring', stiffness: 300, damping: 28 }}>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.22, type: 'spring', stiffness: 260, damping: 28 }} style={{ padding: '0 18px', marginBottom: 26, position: 'relative', zIndex: 1 }}>
        ...
        >
          <div style={{ position: 'absolute', inset: 0, backgroundImage: PAPER_GRAIN_BACKGROUND, pointerEvents: 'none', opacity: 0.35 }} />

          <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', gap: 11 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <p style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: 9, fontWeight: 700, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgba(26,46,26,0.45)' }}>Loyalty Card</p>
              <motion.button
                type="button"
                whileTap={{ scale: 0.92 }}
                onClick={(e) => {
